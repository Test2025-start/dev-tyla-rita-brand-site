// QRCode Landing Page — Main Application Logic
(function() {
  ...
  var navDefaultLabels = {
    home: t('navHome', 'Home'), products: t('navProducts', 'Products'),
    about: t('navAbout', 'About'), contact: t('navContact', 'Contact')
  };
  ...
  if (B.logo) { navLogo.innerHTML = '<img src="'+B.logo+'" alt="'+brandNameResolved+'" style="height:32px;width:auto;">'; }
  ...
  var activeCategory = categories[0] || allCatLabel;
  ...
  if (!products.length) {
    ...
    var allCats = categories.length ? categories.map(function(c){ return l(c, 'allCategory'); }) : [allCatLabel];
  ...
  function renderProducts() {
    ...
    var filtered = activeCategory === allCatLabel ? products : products.filter(function(p) {
      return l(p.category, 'allCategory') === activeCategory;
    });
    ...
    filtered.forEach(function(p) {
      ...
      card.innerHTML =
        '<div class="product-image-wrap">' +
          ...
          (pCat !== activeCategory && activeCategory !== allCatLabel ? '' : '<span class="product-tag">'+pCat+'</span>') +
        '</div>' +
  ...
  if (contact.whatsapp && contact.whatsapp.enabled) {
    hasAnyContact = true;
    var wa = doc.createElement('div'); wa.className = 'contact-card';
  ...
  if (contact.wechat && contact.wechat.enabled) {
    hasAnyContact = true;
    var wc = doc.createElement('div'); wc.className = 'contact-card';
  ...
  if (siteQR.enabled && siteQR.qrImage && siteQR.showOnPage !== false) {
    hasAnyContact = true;
    var sq = doc.createElement('div'); sq.className = 'contact-card';
    sq.innerHTML = '<div class="contact-card-icon" style="background:#FFF8E1;color:#FF6F00;"><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M3 11h8V3H3v8zm2-6h4v4H5V5zm8-2v8h8V3h-8zm6 6h-4V5h4v4zM3 21h8v-8H3v8zm2-6h4v4H5v-4zm13-2h-2v3h-3v2h3v3h2v-3h3v-2h-3z"/></svg></div>' +
  ...
  if (contact.email && contact.email.enabled && contact.email.address) {
    hasAnyContact = true;
    var em = doc.createElement('div'); em.className = 'contact-card';
